...
/* harmony export */   "j": () => (/* binding */ getStockInfo),
/* harmony export */   "W": () => (/* binding */ getAllStocks)
/* harmony export */ });
...
;
const formattedDate = `${year}-${month.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}` //padStart確保月份和日期為兩位數
;
const lastYearDate = `${year - 1}-${month.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`;
const getStockInfo = async (id)=>{
    try {
        let response = await fetch(`${cors}${baseURL}?dataset=TaiwanStockPrice&data_id=${id}&start_date=${lastYearDate}&end_date=${formattedDate}`);
